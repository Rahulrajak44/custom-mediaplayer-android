...
public class PlaybackService extends MediaBrowserServiceCompat implements IVLCVout.Callback {
                    ...
                    try {
                        ...
                        synchronized (mUpdateMeta) {
                            ...
                            if (cover == null || cover.isRecycled())
                                cover = BitmapFactory.decodeResource(ctx.getResources(), R.drawable.app_icon);

                            boolean video = mw.hasFlag(MediaWrapper.MEDIA_FORCE_AUDIO);
                            // add notification to status bar
                            builder = new NotificationCompat.Builder(ctx);
                            builder.setSmallIcon(video ? R.drawable.ic_local_movies_white_36dp : R.drawable.ic_music_note_white_36dp)
    ...
    private void updateWidgetState() {
        ...
        if (hasCurrentMedia()) {
            ...
            widgetIntent.putExtra("title", getString(R.string.widget_default_text, ((VLCApplication)getApplication()).getConfig().getAppName()));
     ...
     */
    @MainThread
    public void loadLocations(List<String> mediaPathList, int position, ProgressDialog progressDialog) {
        ArrayList<MediaWrapper> mediaList = new ArrayList<>();
        locationProgressDialog = progressDialog;
    ...
    public void loadLocation(String mediaPath) {
        loadLocations(Collections.singletonList(mediaPath), 0, null);
    }


    @MainThread
    public void loadLocation(String mediaPath, ProgressDialog progressDialog) {
        loadLocations(Collections.singletonList(mediaPath), 0, progressDialog);
    }
    ...
    @MainThread
    public void load(MediaWrapper media, ProgressDialog progressDialog) {
        locationProgressDialog = progressDialog;
